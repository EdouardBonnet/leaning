...
axioms for planned theorem statements live in `Theorem10Contract.lean`.
-/
...
structure BoundedErrorRefinementPartitionSequence {n m : ℕ}
    (M : _root_.Matrix (Fin n) (Fin m) Bool) (r t : ℕ) where
...
/-- The refinement blow-up in the second item of Theorem 10.
...
nonconstant error value bounded by this expression in the Boolean alphabet
case. -/
...
def theorem10MatrixTwinWidthBound (k : ℕ) : ℕ :=
  theorem10ErrorRefinementBound (theorem10MixedValueBound marcusTardosConstant k)
