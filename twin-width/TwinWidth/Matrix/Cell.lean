import TwinWidth.Order.Divisions
import Mathlib.Data.Matrix.Basic

/-!
# Cells of divided Boolean matrices
...
def CellVertical {n m k : ℕ} (M : _root_.Matrix (Fin n) (Fin m) Bool)
...
def CellHorizontal {n m k : ℕ} (M : _root_.Matrix (Fin n) (Fin m) Bool)
    (R : Division n k) (C : Division m k) (i j : Fin k) : Prop :=
  ∀ ⦃r : Fin n⦄, r ∈ R.part i →
    ∀ ⦃c₁ c₂ : Fin m⦄, c₁ ∈ C.part j → c₂ ∈ C.part j →
      M r c₁ = M r c₂

/-- A cell of a Boolean matrix is mixed when it is neither vertical nor
horizontal, matching Section 5 of the first twin-width paper. -/
def CellMixed {n m k : ℕ} (M : _root_.Matrix (Fin n) (Fin m) Bool)
    (R : Division n k) (C : Division m k) (i j : Fin k) : Prop :=
