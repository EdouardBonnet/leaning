...
/-- The currently imported crossbar theorem also provides the Section 4
weaker input form.  This keeps the weaker target available without duplicating
the stronger proof. -/
theorem exists_crossbarDichotomyInput10 :
    ∃ c : ℕ, 0 < c ∧ CrossbarDichotomyInput10.{u} c :=
  CrossbarTheorem.crossbar_or_strong_pathOfSets_minor_degree10
        ...
        (Hsys : HairyPathOfSetsSystem G ell w),
          2 ≤ g →
            CrossbarContract.IsPowerOfTwo g →
              2 ^ 22 * g ^ 9 * Nat.log 2 g ≤ w →
                (∀ i : Fin ell,
                  _root_.SimpleGraph.HairyCrossbarGrid.OneBasedOdd i →
                  Nonempty (Crossbar (Hsys.hairLocalGraph i)
...
theorem gridMinor_or_strong_pathOfSets_minor_of_hairy_pathOfSets_of_largeCaseCutMatchingDataProvider
    (hlargeData :
      ∃ cGrid : ℕ, 0 < cGrid ∧
        _root_.SimpleGraph.HairyCrossbarGrid.LargeCaseCutMatchingDataProvider.{u}
          cGrid) :
    ∃ cCross cGrid : ℕ, 0 < cCross ∧ 0 < cGrid ∧
      ∀ {V : Type u} [Fintype V] [DecidableEq V]
        (G : _root_.SimpleGraph V) {ell w g : ℕ}
...
theorem gridMinor_or_strong_pathOfSets_minor_of_hairy_pathOfSets_of_fixedRoundLargeCaseCutMatchingDataProvider
    (hfixed :
      ∃ cRound cScale : ℕ, 0 < cRound ∧ 0 < cScale ∧
        _root_.SimpleGraph.HairyCrossbarGrid.FixedRoundLargeCaseCutMatchingDataProvider.{u}
          cRound cScale) :
    ∃ cCross cGrid : ℕ, 0 < cCross ∧ 0 < cGrid ∧
      ∀ {V : Type u} [Fintype V] [DecidableEq V]
        (G : _root_.SimpleGraph V) {ell w g : ℕ}
                          ...
                          g ^ 2 ≤ cCross * ell' ∧
                            g ^ 2 ≤ cCross * w' ∧
                              CrossbarContract.HasStrongPathOfSetsMinor
                                G ell' w' :=
  gridMinor_or_strong_pathOfSets_minor_of_hairy_pathOfSets_of_largeCaseCutMatchingDataProvider
    (_root_.SimpleGraph.HairyCrossbarGrid.exists_largeCaseCutMatchingDataProvider_of_fixedRound
      hfixed)
...
theorem gridMinor_or_strong_pathOfSets_minor_of_hairy_pathOfSets_of_unbundledCutMatching_and_separatorProviders
    (hproviders :
      ∃ cRound cScale : ℕ, 0 < cRound ∧ 0 < cScale ∧
        _root_.SimpleGraph.HairyCrossbarGrid.FixedRoundCutMatchingUnbundledProvider.{u}
          cRound ∧
          _root_.SimpleGraph.HairyCrossbarGrid.FixedRoundSeparatorGridProvider
            cRound cScale) :
    ∃ cCross cGrid : ℕ, 0 < cCross ∧ 0 < cGrid ∧
      ∀ {V : Type u} [Fintype V] [DecidableEq V]
        (G : _root_.SimpleGraph V) {ell w g : ℕ}
                          ...
                          g ^ 2 ≤ cCross * ell' ∧
                            g ^ 2 ≤ cCross * w' ∧
                              CrossbarContract.HasStrongPathOfSetsMinor
                                G ell' w' :=
  gridMinor_or_strong_pathOfSets_minor_of_hairy_pathOfSets_of_largeCaseCutMatchingDataProvider
    (_root_.SimpleGraph.HairyCrossbarGrid.exists_largeCaseCutMatchingDataProvider_of_unbundled_and_separator
      hproviders)
