import «statements-and-proofs».Exponent7.CutResponder.CutResponderAmortizedPipelineV2
import «statements-and-proofs».Exponent8.RootedSection42
import «statements-and-proofs».MinorTransitivity

/-! # Pseudo-grid exit for the existential-routing cut responder -/
...
/-- The complete no-crossbar pseudo-grid branch for V2. -/
theorem gridMinor_of_pseudoGrid_noCrossbar_cleanActiveV2
    (hstrategy :
      ∀ responder :
    ...
    (hreserve : 0 < reserve)
    (Gamma : PseudoGrid G A B X pseudoScale D P Q)
    (hminimal : P.IsMinimumTheorem41Pair Q)
    (hpseudo : 2 ≤ pseudoScale)
    (hpseudoPow : CrossbarContract.IsPowerOfTwo pseudoScale)
    (hNlower : 64 * pseudoScale ^ 4 ≤ Gamma.rowPacking.card)
    (hNupper : Gamma.rowPacking.card ≤ 64 * pseudoScale ^ 6)
    ...
    (hscaledWidth :
      20000 * (reserve * coordinateOrder ^ 2) ≤ pseudoScale ^ 2)
  ...
  have hMpos :
      0 < exponentSevenUniformSlices pseudoScale systemLength :=
    exponentSevenUniformSlices_pos hSystemLength
  have hDpos : 0 < D := by
    have : 0 < 64 * pseudoScale ^ 4 := by positivity
    exact this.trans_le hDscale
  have hDhatPos : 0 < 32 * pseudoScale ^ 4 := by positivity
  have hmass :
      2 * Gamma.rowPacking.card * (4 * pseudoScale ^ 2) ≤
        (32 * pseudoScale ^ 4) * Gamma.rowPacking.card := by
    have hsmall :
        2 * (4 * pseudoScale ^ 2) ≤ 32 * pseudoScale ^ 4 := by
      have hpseudo2 : 1 ≤ pseudoScale ^ 2 :=
        Nat.one_le_pow 2 pseudoScale (by omega)
      nlinarith
    calc
      2 * Gamma.rowPacking.card * (4 * pseudoScale ^ 2) =
          Gamma.rowPacking.card * (2 * (4 * pseudoScale ^ 2)) := by ring
      _ ≤ Gamma.rowPacking.card * (32 * pseudoScale ^ 4) :=
        Nat.mul_le_mul_left Gamma.rowPacking.card hsmall
      _ = (32 * pseudoScale ^ 4) * Gamma.rowPacking.card := by ring
  ...
  have hRcard : Rbar.card = Gamma.rowPacking.card := by simp [Rbar]
  have hRlower : 64 * pseudoScale ^ 4 ≤ Rbar.card := by
    simpa [hRcard] using hNlower
  have hRupper : Rbar.card ≤ 64 * pseudoScale ^ 6 := by
  ...
  let L0' : RecursiveSliceLayer
      ...
      (exponentSevenUniformSlices pseudoScale systemLength)
      Rbar.card (4 * pseudoScale ^ 2) (32 * pseudoScale ^ 4) := by
    simpa [H, Rbar, Qbar, hRcard] using L0
  have hgrid : ContainsGridMinor H target :=
    gridMinor_of_uniformAmortizedPipeline_cleanActiveV2
