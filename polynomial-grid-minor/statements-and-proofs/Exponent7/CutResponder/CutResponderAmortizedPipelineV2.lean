...
import «statements-and-proofs».Exponent7.CutResponder.CutResponderSection5GridV2

/-!
# Uniform amortized pipeline for the V2 cut responder

The proved finite controller is reused verbatim.  Only the terminal
weak-system consumer differs from the frozen pipeline.
-/
...
/-- Uniform source-facing controller with the V2 cut-responder grid exit. -/
theorem gridMinor_of_uniformAmortizedPipeline_cleanActiveV2
    (hstrategy :
      ∀ responder :
  ...
  exact
    CutResponder.AmortizedSlicingDichotomy.gridMinor_of_cleanActiveCutResponderV2
