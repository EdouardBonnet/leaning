...
import «statements-and-proofs».Exponent7.CutResponder.CutResponderPseudoGridV2

/-!
# Local crossbar-or-grid dichotomy for the existential cut responder

This is the routing-existential counterpart of
`localCrossbar_or_grid_cleanActive`.  All scale arithmetic and the crossbar
branch are unchanged; only the no-crossbar consumer uses the corrected V2
interface.
-/
...
/-- Local Chuzhoy--Tan dichotomy with the existential clean active
cut-responder in the no-crossbar branch. -/
theorem localCrossbar_or_grid_cleanActiveV2
  ...
  · by_cases hcross :
    ...
    · rcases hpseudoGrid with ⟨Gamma⟩
      ...
      have hgrid : ContainsGridMinor H target :=
        gridMinor_of_pseudoGrid_noCrossbar_cleanActiveV2
